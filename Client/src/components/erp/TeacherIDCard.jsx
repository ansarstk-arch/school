...
const imgUrl = (path) => {
  ...
  // For teacher images from backend (e.g., "teachers/image.jpg" or bare filename)
  if (trimmed.includes('teachers/')) {
    return `${API_BASE}/uploads/${trimmed}`;
  }
  
  // Bare filename — assume teachers subfolder
  return `${API_BASE}/uploads/teachers/${trimmed}`;
};

/**
 * TeacherIDCard Component
 * Renders a teacher ID card with teacher_id.png background and overlaid data
 * 
 ...
 */
const TeacherIDCard = forwardRef(({ teacher, scale = 1 }, ref) => {
  ...
  return (
    ...
    >
      {/* Background Image - Using teacher_id.png */}
      <img
        src="/teacher_id.png"
        alt="Teacher ID Card Background"
      ...
      {teacher.image && (
        <img
          src={imgUrl(teacher.image)}
          alt={teacher.name || teacher.fullName}
          crossOrigin="anonymous"
          onError={(e) => {
            console.error('Failed to load teacher image:', teacher.image);
      ...
      {teacher.id && (() => {
        try {
          ...
          return (
            ...
            >
              <QRComp
                value={buildAttendanceQrPayload("teacher", teacher.id)}
      ...
      >
        <div style={{ marginBottom: `${40 * scale}px` }}>
          <strong>ایډیکارډ نمبر:</strong> {teacher.id || '—'}
        </div>
        <div style={{ marginBottom: `${40 * scale}px` }}>
          <strong>نوم:</strong> {teacher.name || teacher.fullName || '—'}
        </div>
        <div style={{ marginBottom: `${40 * scale}px` }}>
          <strong>د پلار نوم:</strong> {teacher.fatherName || '—'}
        </div>
        <div style={{ marginBottom: `${15 * scale}px` }}>
          <strong>صلاحیت:</strong> {EDU_LABEL[teacher.education] || EDU_LABEL[teacher.qualification] || teacher.education || teacher.qualification || teacher.className || '—'}
        </div>
      </div>
